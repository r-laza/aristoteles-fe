...
export function Topbar({ studentName, initials, isMenuOpen, onMenuToggle, overBanner = false }: TopbarProps) {
  const glassStyle = overBanner
    ? 'border border-white/40 bg-white/60 shadow-[0_1px_4px_rgb(15_23_42/0.06)] backdrop-blur-md'
    : '';

  return (
    <header className={`relative ${overBanner ? 'bg-transparent' : 'bg-slate-50'}`}>
      <div className="flex h-14 items-center gap-3 px-4 sm:gap-4 sm:px-6 lg:px-8">
        ...
        <div className="ml-auto flex min-w-0 items-center gap-2 sm:gap-3">
          ...
          <div className={`flex h-10 min-w-0 items-center gap-2 rounded-full px-1 sm:pr-3 ${glassStyle}`}>
            ...
            <div className="hidden min-w-0 text-left sm:block sm:max-w-48">
              <div className="truncate text-[13px] font-semibold leading-4 text-slate-900">{studentName}</div>
              <div className="text-[11px] leading-4 text-slate-600">{t('student.role')}</div>
            </div>
          </div>
        </div>
      </div>
